...
export const schedules = {
  ...
  filterFreq: '频率',
  all: '全部',
  // Frequency filter + recurring-row display.
  freq: {
    once: '一次性',
    recurring: '周期',
  },
  recur: {
    ...
    pause: '暂停',
    resume: '恢复',
    pauseFailed: '暂停失败：{{msg}}',
    resumeFailed: '恢复失败：{{msg}}',
    // Short weekday names (Sunday-first, index 0-6 = 周日..周六) for the
    // weekly chip. Kept here so the day CSV (0=Sunday) maps directly by index.
  ...
  modal: {
    ...
    recurTimeLabel: '执行时刻',
    recurTimeHint: '每次触发的本地时刻（与浏览器时区一致，无需换算）',
    recurDaysLabel: '重复星期',
    errWeekdaysRequired: '请至少选择一个星期。',
    errRecurTime: '执行时刻格式不正确（应为 HH:MM）。',
    errRecurDays: '重复星期不正确（应为 0-6，0=周日）。',
    introPrefix: '为「',
    introMiddle: '」设置定时',
    introDesign: '生成技术方案',
    introCoding: '开始开发',
    ...
    agentLabel: '执行环境',
    agentLocal: '本地执行',
    // Design-stage Agent server picker. The control itself is the shared
    // <ExecEnvSelect> (same as coding and the design toolbar), so the server
    // option list renders identically; only these two strings diverge, so the
    // design flow reads "Agent 服务器" / "默认（本地）" rather than the
    // coding-only "执行环境" / "本地执行". Coding keeps using
    // agentLabel/agentLocal to preserve its existing UX.
    designAgentServerLabel: 'Agent 服务器',
    designAgentServerDefault: '默认（本地）',
    designAgentServerLabelMerged: '方案执行环境',
