...
export const components = {
  ...
  createRequirement: {
    ...
    errCreate: '创建失败',
    // 启动计划：创建时就决定「立即/定时执行 + 各阶段模型 + 执行环境 +
    // 是否拆分 + 开发模式」。默认折叠，不展开则完全保持原有创建行为。
    launch: {
      sectionTitle: '⚙ 启动计划（可选）',
      sectionHint: '不展开则只创建需求，不会自动启动',
      modeLabel: '启动方式',
      modeImmediate: '立即执行',
      modeScheduled: '定时执行',
      immediateBlocked: '「完整流程」的需求还没有需求分析会话，无法立即生成方案；请改为「定时执行」，或创建后先完成需求分析。',
      scheduledFullHint: '提示：定时任务触发前请先完成需求分析，否则该次任务会失败。',
      recurrenceLabel: '执行频率',
      designSection: '方案阶段',
      codingSection: '开发阶段',
      execEnvLabel: '执行环境',
      localExec: '本地执行',
      noAgentHint: '暂无就绪的 Agent Server，将在本地执行。',
      readKnowledge: '执行前读取项目知识库',
      splitTasks: '自动拆分为子任务',
      baseBranch: '基线分支',
      newBranch: '开发分支',
      newBranchPlaceholder: '留空自动生成 feat/<需求号>',
      devModeLabel: '开发模式',
      devModeDesign: '基于方案（新会话）',
      devModeSession: '基于会话（续接方案会话）',
      submitImmediate: '创建并开始',
      submitScheduled: '创建并定时',
      dispatchFailed: '需求已创建，但自动启动失败：{{reason}}，请在详情页手动启动。',
      scheduled: '需求已创建，并已排期自动执行。',
    },
  },
