...
// UpdateStageConfig persists the effective Claude-config (claude_configs.id)
// selected for a stage. Mirrors UpdateStageModel — written on the success path
// only so a failed run never clobbers the last good binding — so the config
...
func (s *RequirementService) UpdateArchitectConfig(id, cfg string) error {
	return s.UpdateStageConfig(id, "architect_config_id", cfg)
}

func (s *RequirementService) UpdateDeveloperConfig(id, cfg string) error {
	return s.UpdateStageConfig(id, "developer_config_id", cfg)
}
...
// UpdateDesignBaseSHA stamps the architect-design baseline SHA onto the requirement.
// Called from the design stage prologue (right after the hard sync succeeds) so a
// failed claude run still records the SHA it tried to design against. The merge
// stage surfaces this in the PR body. Mirrors UpdateDesignAgentServer above.
func (s *RequirementService) UpdateDesignBaseSHA(id, sha string) error {
	_, err := s.db.Exec(
		"UPDATE requirements SET design_base_sha = ?, updated_at = ? WHERE id = ?",
		sha, time.Now(), id)
	return err
}
