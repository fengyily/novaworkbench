...
# ownership of bind-mounted state directories so the non-root nova user
# (UID 1000; see Dockerfile) can read/write them, then drops privileges
# with su-exec and execs the server.
#
# Why this exists:
...
#   nova user can write to the DB / clone repos inside the container.
#
...
# Directories that must be writable by nova. These mirror the volume
# mount targets in docker-compose.yml and deploy/docker-compose.prod.yml.
DATA_DIR="${NOVA_HOME:-/home/nova}/.novaworkbench"
WORK_DIR="${NOVA_WORK:-/home/nova}/workspace"

mkdir -p "$DATA_DIR" "$WORK_DIR"

# Backward-compat: if an older deployment wrote data to /root/... and
# /home/nova/... is empty, migrate it so users don't lose their projects.
...
# XDG lookups inside the nova processes.
export HOME="${NOVA_HOME:-/home/nova}"
exec su-exec nova env "HOME=$HOME" "$@"
