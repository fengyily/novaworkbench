...
Env vars the backend reads: `NOVA_PORT` (default `9527`), `CLAUDE_BIN` (default `claude`), `CLAUDE_TIMEOUT` (default `120s`; single-shot CLI prompts), `CLAUDE_CODING_TIMEOUT` (coding-task timeout, used when the 设置→Claude 配置 value is unset; default `2h`), `NOVA_DB_DRIVER` (`sqlite` default | `mysql` | `postgres`), `NOVA_DB_DSN`, `NOVA_DB_PATH` (sqlite file, default `~/.novaworkbench/data/nova.db`). The frontend reads `VITE_API_BASE` (default `http://localhost:9527`).
...
`GenerateCode` returns an unstarted `*exec.Cmd` plus a `context.CancelFunc` (call it after the run is reaped to release the timeout timer) so the **handler** owns the lifecycle and parses the stream itself — this is the pattern for anything that needs live progress. Its timeout is resolved per call via `CodingTimeoutProvider` (`SettingService.CodingTimeout()`): settings-table `llm.coding_timeout` → `CLAUDE_CODING_TIMEOUT` env → `2h` default. The value is editable via `GET/PUT /api/settings/coding-timeout` (设置→Claude 配置→编码超时).
...
4. `start-coding` — creates a JobStore job, checks out the dev branch, and runs `llm.GenerateCode` in a goroutine, streaming `tool_call`/`message`/`tool_result`/`done` events into the job. On `job_done` the frontend sets status `developing`; the user manually marks `done`. Coding runs get a configurable timeout (default `2h`); on timeout the process is killed and the job finishes as `error`, and the UI offers **继续开发** (`--resume coding_session_id` via `adjust-coding` with a fixed "please continue" prompt) alongside **重新开发** (`--fork-session` from the design session). The finished job's full log is persisted to `job_logs`; a 30s checkpoint also snapshots the in-progress `running` state so a backend restart mid-run doesn't lose the whole log, and a run killed before its `result` event records a `{"interrupted":true}` token-usage marker row.
