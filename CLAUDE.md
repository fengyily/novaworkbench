...
- **Frontend**: React 19 + TypeScript 6 + Vite 8 + React Router v7. **i18n**: `i18next ^26` + `react-i18next ^17` (`frontend/src/i18n/`, 16 zh-CN/en-US module pairs under `frontend/src/i18n/locales/modules/`, LanguageSwitcher in Layout/Login). Lint: `oxlint`.
...
**27 tables** (`schema.go:14,31,46,74,89,109,116,123,137,147,160,166,178,191,202,225,243,255,265,273,281,291,300,317,350,389,415`):
...
4. **`start-coding`** — creates a JobStore job, checks out the dev branch, and runs `llm.GenerateCode` in a goroutine, streaming `tool_call`/`message`/`tool_result`/`done` events into the job. On `job_done` the frontend sets status `developing`; the user manually marks `done`. **`start-coding-immediate`** (LaunchSpec 入口,由 `DesignCodingImmediateModal` 触发)跳过手动 refine-doc,把 design+coding 一次性派发到 JobStore。
...
- **打包**: `packaging/nfpm.yaml`(nfpm 出 **deb 给 apt 源**,见 `.github/workflows/release.yml:201` 注释,未输出 rpm);`apt` 源配置见 `README.md → apt 安装`。
- **Release + CI/CD**: `docs/RELEASE.md` 详述 Release Please + ldflags 注入 `version.Version`;`.github/workflows/{deploy,terraform,release,release-please}.yml` 跑 CI/CD(`release-please.yml` v0.5.0 起接管自动 PR);`terraform/` IaC prod provisioning。
...
- **API 客户端 (`frontend/src/api/`)** — **单文件 `client.ts` (2,276 行)** 集中 27 个 `*Api` 对象 + `api.get/post/put/patch/delete` 包装;`stream.ts` (93 行) 是 `createEventStream()` SSE 替代品。27 个 `*Api`:`projectsApi` / `dashboardApi` / `fsApi` / `memoriesApi` / `knowledgeApi` / `scannerApi` / `subTasksApi` / `subTaskConfigApi`(v0.5.0 新增) / `requirementsApi` / `wizardApi` / `runnerApi` / `mergeApi` / `reviewApi` / `platformApi` / `claudeApi` / `llmApi` / `databaseApi` / **`rolesApi`(AI persona)** / `skillsApi` / `reportsApi` / `usageApi` / `authApi` / **`aclApi`(RBAC, 含义与 `rolesApi` 不同)** / `preflightApi` / `agentServersApi` / `schedulesApi` / `gitSyncConfigApi`(v0.5.0 新增).
- **路由表 (`App.tsx:55-83`, `RequireAuth` + `<Layout>` 包裹)**: `/login` `Login`; `/` `Dashboard`; `/wizard` `WizardPage`; `/projects/add` `AddProject`; `/projects` `ProjectsList`; `/projects/:id` `ProjectDetail`(内嵌 `/projects/:id/reports` 由 `ProjectWeeklyReport` 渲染); `/requirements` `RequirementsList`; `/requirements/calendar` `RequirementsCalendar`; `/requirements/:id` `RequirementDetail`; `/schedules` `SchedulesPage`; `/knowledge` `KnowledgePage`; `/chat` `Chat`(占位); `/reports` `Reports`(占位); `/settings` `Settings`(嵌套 `index=SettingsTokens` / `users=SettingsUsers` / `acl=SettingsACLRoles` / `roles=SettingsRoles` / `agent-servers=SettingsAgentServers` / `skills=SettingsSkills` / `claude=SettingsClaude` / `llm=SettingsLLM` / `subtask=SettingsSubTask` / `git-sync=SettingsGitSync` / `database=SettingsDatabase` / `preflight=SettingsPreflight`)。
...
- **components/** + **utils/**: `Layout` + `FolderPicker` + 三个 Chat(`DeepRefineChat`/`DocRefineChat`/`CodingChat`)消费 SSE over `fetch` + 手动 `ReadableStream`;另含 `AtMentionTextarea` / `ContextUsageBar` / `ExecEnvBadge` / `ExecEnvSelect` / `FullscreenButton` / `MarkdownViewer` / `ModelSelect` / `ScheduleModal` / `SessionContextStrip` / `StatusChips` / `SubTaskPanel` / `SummarizeToRequirementModal` / `DesignCodingImmediateModal`(v0.5.0 新增) / `DevSourceBadge`(v0.5.0 新增),子目录 `calendar/` + `icons/` + `CreateRequirementForm/`;utils 含 13 个工具文件 `auth.tsx`(`AuthProvider`)/ `errMsg.ts` / `exportDesignPdf.tsx` / `intl.ts` / `lang.ts` / `logLines.ts` / `modelConfig.ts` / `modelWindow.ts` / `phaseGroups.ts` / `preview.ts` / `statusChips.ts` / `time.ts` / `useFullscreen.ts`。
